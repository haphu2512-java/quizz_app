...
public class Login extends JFrame {
    ...
    // Use a UserServiceImpl for business logic
    private UserServiceImpl userService;

    public Login() {
        try {
            // Instantiate the service. In a real app, this would be injected.
            ...
            System.exit(1); // Exit if service cannot be initialized
        }
        ...
        buttonLoginViewLogin.addActionListener(event -> {
            ...
            if (username.isEmpty() || password.isEmpty()) {
                ...
                if (username.equals(username_admin) && password.equals(password_admin)) {
                    // For admin, directly create a User entity and encrypt the password
                    var admin = new User(username_admin, username_admin, UserDAO.encryptPassword(password_admin), true);
                    this.dispose();
                    new MenuAdmin(admin);
                } else {
                    try {
                        var loginUser = userService.selectByAccount(username, password); // Use service layer
                        if (loginUser != null) {
                            this.dispose();
                            var checkHost = loginUser.isHost(); // Use getter from entity.User
                            if (checkHost) {
